...
export function DeviceDetailDrawer() {
  const { selectedDevice, drawerOpen, screenshotData, deviceApps, loadingScreenshot, loadingApps, closeDrawer, connectDevice, disconnectDevice, enableWireless, enablingWireless } = useDeviceStore();
  ...
  return (
    <>
      ...
      <div className="fixed right-0 top-0 h-full w-[400px] bg-[#1e293b] border-l border-[#334155] z-50 animate-slide-in-right overflow-y-auto">
        ...
        <div className="p-4 border-b border-[#334155]">
          <h3 className="text-[#94a3b8] text-sm font-medium mb-3">已安装应用</h3>
          {loadingApps ? (
            <div className="flex items-center justify-center py-8">
              <Loader2 className="w-6 h-6 text-indigo-400 animate-spin" />
            </div>
            ...
            <div className="space-y-2 max-h-[200px] overflow-y-auto">
              {deviceApps.map((app) => (
                <div
                  ...
                  className="flex items-center justify-between p-2 bg-[#0f172a] rounded-lg text-sm"
                >
                  <span className="text-[#94a3b8]">{app.name || app.package_name}</span>
                  <span className="text-[#64748b] font-mono text-xs">{app.package_name}</span>
                </div>
              ))}
            </div>
          ) : (
            <div className="text-center py-8 text-[#64748b] text-sm">暂无数据</div>
          )}
        </div>
