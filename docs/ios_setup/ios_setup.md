...
本文档介绍如何为 Open-AutoPhone 配置 iOS 设备环境。
...
1. 在 Xcode 中选中 `WebDriverAgent`，出现 General、Signing & Capabilities 等选项卡。
...
需要满足以下条件：
1. 通过 USB 连接。在 Finder 中选中连接的 iPhone，在"通用"中勾选"WiFi 中显示这台 iPhone"。
2. Mac 和 iPhone 处于同一 WiFi 网络之下。
...
其中 **http://[设备IP]:8100** 为 WiFi 所需的 WDA_URL。
...
之后，打开一个新终端，通过以下命令使用 AutoPhone（WiFi 则使用上述获得的 WDA_URL）：
...
- [PR141](https://github.com/zai-org/Open-AutoPhone/pull/141)
- [Gekowa 提供的 iOS 方案](https://github.com/gekowa/Open-AutoPhone/tree/ios-support)
